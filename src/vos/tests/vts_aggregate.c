		   ...
		   daos_epoch_t punch_epoch[], daos_epoch_t punch_bound[])
{
	...
	if (ds->td_discard)
		...
		rc = vos_aggregate(arg->ctx.tc_co_hdl, epr_a,
				   ds_csum_agg_recalc, NULL, NULL, NULL, false);
	if (rc != -DER_CSUM) {
...

{
	...
	if (ds_sample->td_discard)
		...
		rc = vos_aggregate(arg->ctx.tc_co_hdl, epr_a, NULL, NULL, NULL, NULL,
				   false);
	assert_rc_equal(rc, 0);
...
aggregate_14(void **state)
{
	...
	for (i = 0; i < repeat_cnt; i++) {
		...
		epr.epr_hi = epc_hi;
		rc = vos_aggregate(arg->ctx.tc_co_hdl, &epr, NULL, NULL, NULL, NULL,
				   false);
		if (rc) {
...
aggregate_22(void **state)
{
	...
	rc = vos_aggregate(arg->ctx.tc_co_hdl, &epr, NULL, NULL, NULL, NULL, false);
	assert_rc_equal(rc, 0);
