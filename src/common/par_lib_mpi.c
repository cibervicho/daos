...
int
par_gather(const void *sendbuf, int sendcount, void *recvbuf, int recvcount, enum par_type type,
	   int root)
{
	MPI_Datatype	mtype = type_par2mpi(type);
	int		rc;
	...
	rc = MPI_Gather(sendbuf, sendcount, mtype, recvbuf, recvcount, mtype, root, MPI_COMM_WORLD);
...
int
par_allgather(const void *sendbuf, int sendcount, void *recvbuf, int recvcount, enum par_type type)
{
	MPI_Datatype	mtype = type_par2mpi(type);
	int		rc;
	...
	rc = MPI_Allgather(sendbuf, sendcount, mtype, recvbuf, recvcount, mtype,
			   MPI_COMM_WORLD);
