...
class TestWithTelemetryIOLatency(IorTestBase, TestWithTelemetry):
    ...
    def ior_with_transfer_size(self, transfer_size, ior_metrics, operation)
        """Run IOR command with specified tranfer size to verify IO metrics.
        ...
        # Set up cmd flags for read or write and tranfer size
    ...
    def verify_io_dtx_committed_metrics(self, metrics_data, test_metrics, transfer_size):
            ...
            for host in self.hostlist_servers:
                ...
                for rank in self.server_managers[-1].get_host_ranks([host]):
                    for target in range(self.server_managers[-1].get_config_value("targets")):
                        value = metrics_data[test_metric][host][str(rank)][str(target)][size]
                        metrics[test_metric] = metrics[test_metric] + value
        value = metrics["engine_io_dtx_committed"]
        ...
        self.log.info("IO DTX committed = %s", value)
        self.log.info("IO DTX committed min = %s", min_value)
        self.log.info("IO DTX committed max = %s", max_value)
        self.log.info("IO DTX committed mean = %s", mean_value)
        self.log.info("IO DTX committed stddev = %s", stddev_value)

        # DTX committed value should be ~ equal to the number of transactions
        num_operations = block_size / transfer_size
        self.log.info("block_size = %s", block_size)
        self.log.info("transfer_size = %s", transfer_size)
        self.log.info("num_operations = %s", num_operations)

        return True

    def test_ior_dtx_telmetry_metrics(self):
        """JIRA ID: DAOS-7316.
        ...
        for transfer_size in transfer_sizes:
            for operation in ["rw"]:
                ...
                if self.verify_io_dtx_committed_metrics(metrics_data, committed_test_metrics,
                                                        str(transfer_size)):
                    self.log.info("IO dtx committed metrics verified for transfer size %s", transfer_size)
                else
                    self.log.error("IO dtx committed metrics failed verification for transfer size %s", transfer_size)
