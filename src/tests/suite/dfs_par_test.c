...
check_one_success(int rc, int err)
{
	...
	par_allgather(&rc, 1, rc_arr, 1, PAR_INT);
...
file_atomicity_test_helper(test_arg_t *arg, int rf)
{
	...
	par_gather(&oid1.hi, 1, oids_hi, 1, PAR_UINT64, 0);
	par_gather(&oid1.lo, 1, oids_lo, 1, PAR_UINT64, 0);
	...
	par_gather(&oid2.hi, 1, oids_hi, 1, PAR_UINT64, 0);
	par_gather(&oid2.lo, 1, oids_lo, 1, PAR_UINT64, 0);
